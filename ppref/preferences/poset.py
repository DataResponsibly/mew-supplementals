...
class Poset(object):
    ...
    def iterate_linear_extensions(self) -> Iterator[list]:
        dag_full = self.dag.copy()
        for e in self.item_set:
            if e not in self.items_in_poset:
                dag_full.add_node(e)

        return nx.all_topological_sorts(dag_full)
