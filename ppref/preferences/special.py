    ...
    def calculate_rank2prob_for_item(self, item):
        ...
        for j in range(m):
            if j < k_l or m - 1 - j < k_r:
                ...
                probs.append(comb4_cached(j, k_l, m - 1 - j, k_r))
...
class PartitionedWithMissing(object):
    ...
    def has_item(self, item):
        return item in self.item_to_bucket
            ...
            for j in range(self.m):
                if k_l <= j <= self.m - 1 - k_r:
                    ...
                    for k_same_bucket_to_left in range(min(j - k_l, b_size - 1) + 1):
                        ...
                        prob += comb4_cached(j, num_bucket_items_left, self.m - 1 - j, num_bucket_items_right)
...
def comb4_cached(a: int, b: int, c: int, d: int) -> int:
    return comb_cached(a, b) * comb_cached(c, d)


@lru_cache(maxsize=100_000)
def comb_cached(a: int, b: int) -> int:
    return comb(a, b)
...
class TruncatedRanking(object):
    ...
    def generate_a_random_instance(cls, top_k: int, bottom_k: int, items: set, rng: NumpyRNG = None):
        ...
        ranking = list(rng.permutation(x=list(items)))
