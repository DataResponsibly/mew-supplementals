...
def show_scalability(m=10, cardinality=5, max_rank=None, verbose=True):
    ...
    t1 = time()
    res = calculate_rank_probs_for_item_given_rim_w_poset_by_sequential(item, mallows, poset)
    t2 = time()

    if verbose:
        probs_sum = sum(res["probabilities"])
        probabilities = [round(prob / probs_sum, ndigits=3) for prob in res["probabilities"]]
        print(f'\nEvaluating item {item}')
        print(mallows)
        print(poset)
        print(f'when {m=} and {cardinality=}, time_sequential={t2 - t1:.3f} seconds, {probabilities=}')
