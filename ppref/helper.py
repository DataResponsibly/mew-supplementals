...
def calculate_kendall_tau_distance_by_enumeration(x: Sequence, y: Sequence):
    assert len(x) == len(y)

    ye2rank = {e: i for i, e in enumerate(y)}

    dist = 0
    for i, ei in enumerate(x[:-1]):
        ei_rank_in_y = ye2rank[ei]
        for j, ej in enumerate(x[i + 1:]):
            ej_rank_in_y = ye2rank[ej]

            if ei_rank_in_y > ej_rank_in_y:
                dist += 1
